...
This invariant is enforced by: the `LLMAdapter` interface, the system prompt in M5,
...
Short version: no auth, no basket, no payments, no real DB, no vector DB, no RAG,
no agent framework, no Docker, no multi-turn memory, no i18n, no Notino-scale catalog.

## Related docs

- Architecture details → [ARCHITECTURE.md](ARCHITECTURE.md)
- Current slice → [VERTICAL_SLICE.md](VERTICAL_SLICE.md)
